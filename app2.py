...
def verificar_login(usuario, senha):
    ...
    return ((usuarios['usuario'] == usuario) & (usuarios['senha'] == senha_hash)).any()

def cadastrar_usuario(novo_usuario, nova_senha):
    senha_hash = hash_senha(nova_senha)
    novo = pd.DataFrame([[novo_usuario, senha_hash]], columns=["usuario", "senha"])
    if os.path.exists("usuarios.csv"):
        usuarios = pd.read_csv("usuarios.csv")
        if novo_usuario in usuarios["usuario"].values:
            return False
